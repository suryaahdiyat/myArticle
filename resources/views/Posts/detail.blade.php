<x-layout>
                ...
                <div>
                    ...
                    @else
                        @foreach ($comments as $comment)
                            {{-- item --}}
                            <div class="relative p-2 my-1 border">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center ">
                                        ...
                                        @else
                                            <div
                                                class="flex items-center justify-center text-xs text-white rounded-full w-7 h-7 bg-slate-500">
                                                {{ $comment->user->initial }}
                                            </div>
                                        @endif
                                        <div class="ml-2 text-start">
                                            <h1 class=""><a href="/postsBy/{{ $comment->user->email }}"
                                                    class="text-sm underline">{{ $comment->user->name }}
                                                </a></h1>
                                            <p class="text-xs font-light">{{ $comment->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                    {{--  tengah --}}
                                ...
                                @auth
                                    ...
                                    <div class="absolute w-6 top-2 right-2 hover:cursor-pointer group">
                                        <div class="flex items-center justify-around" onclick="handleClickDel()">
                                            <div class="w-1 h-1 duration-100 bg-gray-700 rounded-full group-hover:-translate-x-2"></div>
                                            <div class="w-1 h-1 duration-100 delay-75 bg-gray-700 rounded-full group-hover:-translate-x-2"></div>
                                            <div class="w-1 h-1 duration-100 delay-150 bg-gray-700 rounded-full group-hover:-translate-x-2"></div>
                                        </div>
                                        <div class="relative text-rose-700">
                                            <form id="btnDel" class="absolute hidden p-2 bg-gray-800 rounded shadow-md top-1 right-1" method="POST" action="/comments/{{$comment->id}}">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('Are you sure want to delete this comment?')">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                        stroke-width="1.5" stroke="currentColor" class="size-6 group-hover:text-rose-500">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <script>
        ...
        const handleClickDel = () => {
            document.getElementById('btnDel').classList.toggle('hidden')
        }
